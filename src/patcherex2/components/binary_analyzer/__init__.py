...
@final
class BasicBlock(FrozenPatcherexModel):
    """A contiguous basic block reported by a binary analyzer."""

    start: Annotated[int, Field(ge=0)]
    size: Annotated[int, Field(gt=0)]
    instruction_addrs: Annotated[
    ...
    def validate_layout(self) -> BasicBlock:
        """Ensure instruction addresses are ordered and inside the block."""
        if self.instruction_addrs != tuple(sorted(set(self.instruction_addrs))):
            raise ValueError("basic block instruction addresses must be unique and sorted")
        if any(address < self.start or address >= self.end for address in self.instruction_addrs):
            raise ValueError("basic block instruction addresses must lie within its bounds")
        return self


class FunctionInfo(FrozenPatcherexModel):
    """Address and extent of a function reported by a binary analyzer."""
...
class BinaryAnalyzer(Protocol):
    ...
    Addresses crossing this interface are *normalized*: relative to the image base for position-
    independent binaries, absolute otherwise. Backends translate to and from their own address
    space with :meth:`normalize_addr` / :meth:`denormalize_addr`.
    """
    ...
    def get_basic_block(self, addr: int) -> BasicBlock:
        """Return the normalized basic block containing ``addr``."""
        ...
    ...
    def get_unused_funcs(self) -> list[FunctionInfo]:
        """Return normalized functions with no references to them."""
        ...
    ...
    def get_all_symbols(self) -> dict[str, MappedSymbol]:
        ...
        Only :class:`MappedSymbol` appears: a name with no address cannot resolve a reference, and
        offering one would hand out a placeholder. The symbol itself is returned rather than just
        its address so a caller can tell a trampoline from a definition.

        A name may legitimately be reported more than once -- typically a trampoline alongside what
        it forwards to. Where the addresses agree the duplicate is redundant; where they differ,
        the non-stub wins, since that is where the thing named actually lives. A name with two non-
        stub addresses is genuinely ambiguous, so it is dropped rather than resolved arbitrarily,
        and a patch referring to it will fail to resolve instead of silently reaching the wrong one.
        """
        ...
        for symbol in self.iter_symbols():
            ...
            if existing := best.get(symbol.name):
                if not symbol.is_stub:
                    ...
                    elif existing.addr != symbol.addr:
    ...
    def get_function(self, name_or_addr: int | str) -> FunctionInfo | None:
        """Return the normalized function named or containing the input, if any."""
        ...
