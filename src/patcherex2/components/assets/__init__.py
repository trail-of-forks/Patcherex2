...
class AssetResolver(Protocol):
    """Resolve a named toolchain asset to its local directory."""

    def resolve(self, name: str) -> Path:
        """Return the local directory containing ``name``."""
        ...
...
class Assets:
    ...
    def download(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with requests.get(str(self.url), stream=True, timeout=60) as r:
                r.raise_for_status()
            ...
            if actual != self.sha256:
                raise AssetIntegrityError(
                    f"sha256 mismatch for {self.name}: expected {self.sha256}, got {actual}"
                )
            with tarfile.open(archive_path) as tar:
                # Python 3.11 lacks tarfile's extraction filter. Integrity is
                # enforced above against the pinned archive digest.
                tar.extractall(path=self.ASSETS_DIR / self.name)


@final
class DownloadingAssetResolver:
    """Resolve bundled assets, downloading and validating them when absent."""

    def resolve(self, name: str) -> Path:
        return Path(Assets(name).path)


DEFAULT_ASSET_RESOLVER = DownloadingAssetResolver()
