...
ARM = ArchitectureInfo(
    ...
    callee_saved={"Linux": ["r4", "r5", "r6", "r7", "r8", "r9", "r10", "r11", "r13", "r14", "r15"]},
    cc_float={"Linux": ["d0", "d1", "d2", "d3", "d4", "d5", "d6", "d7"]},
    callee_saved_float={"Linux": ["d8", "d9", "d10", "d11", "d12", "d13", "d14", "d15"]},
    float_types={32: "float", 64: "double"},
    subregisters={f"r{i}": {32: [f"r{i}"]} for i in range(16)},
    subregisters_float={
        **{f"d{i}": {64: [f"d{i}"], 32: [f"s{2 * i}"]} for i in range(16)},
        **{f"d{i}": {64: [f"d{i}"]} for i in range(16, 32)},
    },
)
