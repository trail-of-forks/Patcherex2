...
class Tests(IntegrationTestMixin):
    ...
    def setup(self, request):
        request.cls.bin_location = str(TEST_BINARIES / "amd64")
        request.cls.binary_analyzer = request.param

    def test_raw_file_patch_nopie(self):
        self.run_one(
            "printf_nopie",
            [ModifyRawBytesPatch(0x2004, b"No", addr_type=AddressType.FILE)],
    ...
    def test_raw_file_patch_pie(self):
        self.run_one(
            "printf_pie",
            [ModifyRawBytesPatch(0x2007, b"No", addr_type=AddressType.FILE)],
            expected_output=b"No",
            expected_return_code=0,
        )
    ...
    def test_insert_instruction_patch_relocates_long_conditional_branch(self):
        filepath = Path(self.bin_location) / "issue37"
    ...
    def test_insert_instruction_patch_c(self):
        ...
        self.run_one(
            ...
            [
                InsertInstructionPatch(
                    0x1157, instrs, language=InstructionPatchLanguage.C, c_config=config
                )
            ],
    ...
    def test_insert_instruction_patch_c_asm_header(self):
        ...
        self.run_one(
            ...
            [
                InsertInstructionPatch(
                    0x130D, instrs, language=InstructionPatchLanguage.C, c_config=config
                )
            ],
    ...
    def test_insert_instruction_patch_c_float(self):
        ...
        self.run_one(
            ...
            [
                InsertInstructionPatch(
                    0x1175, instrs, language=InstructionPatchLanguage.C, c_config=config
                )
            ],
            expected_output=expected_output,
            expected_return_code=0,
