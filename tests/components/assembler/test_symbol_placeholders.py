...
from patcherex2.components.assembler import (
    Assembler,
...
class _FakeAssemblyBackend:
    def assemble(self, code: str, base: int = 0, *, is_thumb: bool = False) -> bytes:
        raise AssertionError("symbol-resolution tests do not assemble code")


class _RecordingAssemblyBackend:
    def __init__(self) -> None:
        self.calls: list[tuple[str, int, bool]] = []

    def assemble(self, code: str, base: int = 0, *, is_thumb: bool = False) -> bytes:
        self.calls.append((code, base, is_thumb))
        return b"assembled"
...
def assembler():
    analyzer = FakeBinaryAnalyzer(
        symbols={name: MappedSymbol(name=name, addr=addr) for name, addr in SYMBOLS.items()}
    )
    return Assembler(SymbolResolver(analyzer), _FakeAssemblyBackend())
...
def test_caller_symbols_take_priority(assembler):
    resolved = assembler.resolve_symbols("b <return_0x32>", symbols={"return_0x32": 0x1234})
    assert resolved == "b 0x1234"


def test_assembler_resolves_symbols_before_delegating_to_backend():
    backend = _RecordingAssemblyBackend()
    analyzer = FakeBinaryAnalyzer(
        symbols={name: MappedSymbol(name=name, addr=addr) for name, addr in SYMBOLS.items()}
    )
    assembler = Assembler(SymbolResolver(analyzer), backend)

    result = assembler.assemble("b <return_0x32>", 0x1000, is_thumb=True)

    assert result == b"assembled"
    assert backend.calls == [("b 0x400100", 0x1000, True)]


def test_empty_assembly_does_not_invoke_backend():
    backend = _RecordingAssemblyBackend()
    analyzer = FakeBinaryAnalyzer(
        symbols={name: MappedSymbol(name=name, addr=addr) for name, addr in SYMBOLS.items()}
    )
