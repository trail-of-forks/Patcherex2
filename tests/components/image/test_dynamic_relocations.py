...
from patcherex2.components.image.elf import (
    DynamicRelocationTableInfo,
    ElfImageBackend,
    pack_rel_entry,
)
...
def test_dynamic_relocation_scan_returns_typed_metadata():
    binary = str(TEST_BINARIES / "amd64/printf_pie")
    backend = ElfImageBackend(binary)
    tags = _dyn_tags(backend._elf)

    info = backend._scan_dynamic_rel_tags()

    assert isinstance(info, DynamicRelocationTableInfo)
    assert info.is_rela is True
    assert info.address == tags["DT_RELA"]
    assert info.size == tags["DT_RELASZ"]
    assert info.entry_size == tags["DT_RELAENT"]
    assert info.relative_count == tags["DT_RELACOUNT"]
    assert info.address_tag == "DT_RELA"
    assert info.size_tag == "DT_RELASZ"
    assert info.relative_count_tag == "DT_RELACOUNT"


def _dyn_tags(elf: ELFFile) -> dict[str, int]:
    if (section := elf.get_section_by_name(".dynamic")) and isinstance(section, DynamicSection):
        return {t.entry.d_tag: t.entry.d_val for t in section.iter_tags()}
    else:
        return {}
...
def _rela_file_off(elf: ELFFile, tags: dict[str, int]) -> int:
    ...
    for s in elf.iter_segments():
        h = s.header
        if h.p_type == "PT_LOAD" and h.p_vaddr <= tags["DT_RELA"] < h.p_vaddr + h.p_filesz:
...
class TestEmitDynamicRelocations:
    PIE = str(TEST_BINARIES / "amd64/printf_pie")

    def _patch(self, tmp_path, relocs) -> str:
        out = str(tmp_path / "patched")
        with PatchSession.load_binary(
            TEST_BINARIES / "amd64/printf_pie",
            target=ELF_AMD64_LINUX,
            config=TargetConfig(selections=ComponentSelections(binary_analyzer="angr")),
        ) as p:
            assert isinstance(p.image, ElfImageBackend)
            if relocs:
                p.image.add_dynamic_relocations(relocs)
            p.apply_patches()
            p.save_binary(out)
        os.chmod(out, 0o755)
        return out
    ...
    def test_rebuilt_rela_is_structurally_valid(self, tmp_path):
        ...
        assert all(
            (e[1] & 0xFFFFFFFF) == R_X86_64_RELATIVE for e in entries[: after["DT_RELACOUNT"]]
        )
