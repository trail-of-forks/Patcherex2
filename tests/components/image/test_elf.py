...
def test_new_segment_preserves_minimum_load_address_offset():
    elf = ElfImageBackend(str(TEST_BINARIES / "amd64/printf_nopie"))
    elf._segments = [
...
def test_program_headers_follow_required_load_order():
    ...
    ordered = ElfImageBackend._order_program_headers(segments)
    ...
    assert [segment["p_vaddr"] for segment in ordered if segment["p_type"] == "PT_LOAD"] == [
        0x400000,
        0x410DF8,
    ]
...
def test_contained_load_range_does_not_shrink_merged_extent():
    assert ElfImageBackend._merge_overlapping_ranges(
